...
function watchRootStats() { ['quotes','cotacoes','suppliers','fornecedores','priceDb','whatsappQueue','respostas','purchaseOrders','ordensCompra','publicCotacoes'].forEach(function(k) { db.ref(k).on('value', function(s) { rootStats[k] = s.numChildren ? s.numChildren() : Object.keys(s.val() || {}).length; renderDiag(); }); }); }
...
async function migrateRootToTenant() { const id = slug($('tenantId').value.trim()); if (!id) { alert('Informe o tenantId que receberá os dados atuais.'); return; } if (!confirm('Copiar dados raiz atuais para tenants/' + id + '? O banco raiz será preservado.')) return; const nodes = ['suppliers','fornecedores','clientes','veiculos','priceDb','quotes','cotacoes','respostas','whatsappQueue','purchaseOrders','ordensCompra','audit','robotStatus']; const updates = {}; const settingsSnap = await db.ref('settings/main').once('value'); if (settingsSnap.exists()) updates['tenants/' + id + '/settings'] = settingsSnap.val(); for (const n of nodes) { const snap = await db.ref(n).once('value'); if (snap.exists()) updates['tenants/' + id + '/' + n] = snap.val(); }
  const publicSnap = await db.ref('publicCotacoes').once('value'); if (publicSnap.exists()) { const publicData = publicSnap.val() || {}; Object.entries(publicData).forEach(function(e) { updates['publicQuotes/' + id + '/' + e[0]] = e[1]; }); }
  await db.ref().update(updates); alert('Migração copiada para tenants/' + id + '. Valide no index.html antes de aposentar os nós raiz.'); }
function renderDiag() { const rootHtml = Object.entries(rootStats).map(function(e) { return '<li>' + esc(e[0]) + ': ' + esc(e[1]) + '</li>'; }).join(''); $('diagBox').innerHTML = '<ul><li>Admin: ' + esc(adminUser && adminUser.email || '') + '</li><li>Tenants cadastrados: ' + Object.keys(tenants || {}).length + '</li><li>Nichos globais: ' + Object.keys(globalDb || {}).length + '</li><li>Nichos disponíveis: ' + NICHOS.length + '</li></ul><h3>Banco raiz atual detectado</h3><ul>' + (rootHtml || '<li>Sem dados raiz carregados.</li>') + '</ul>'; }
