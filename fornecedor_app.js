...
function publicPath() { return 'publicQuotes/' + tenantId + '/' + quoteId; }
function responseBasePath() { return 'tenants/' + tenantId + '/quotes/' + quoteId + '/responses/' + supplierId; }
window.addEventListener('load', function() {
  firebase.initializeApp(window.firebaseConfig);
  db = firebase.database();
  const p = new URLSearchParams(location.search);
  tenantId = p.get('t') || p.get('tenantId') || '';
  quoteId = p.get('q') || p.get('quoteId') || '';
  supplierId = p.get('s') || p.get('supplierId') || p.get('fornecedor') || '';
  if (!tenantId || !quoteId || !supplierId) { block('Link inválido. Peça ao estabelecimento para reenviar a cotação.'); return; }
  loadQuote();
});
...
async function saveItem(finish) {
  ...
  updates[publicPath() + '/lastResponseAt'] = Date.now();
