...
function watchQueue(ctx) { const qPath = queuePathFor(ctx); if (watchedPaths.has(qPath)) return; watchedPaths.add(qPath); db.ref(qPath).orderByChild('status').equalTo('pending').on('child_added', snap => processQueueItem(ctx, snap.key, snap.val() || {})); db.ref(qPath).orderByChild('status').equalTo('pendente').on('child_added', snap => processQueueItem(ctx, snap.key, snap.val() || {})); setInterval(() => scanPending(ctx), Math.max(15000, Number(roboConfig.sendIntervalMs || 9000) * 2)); scanPending(ctx); }
